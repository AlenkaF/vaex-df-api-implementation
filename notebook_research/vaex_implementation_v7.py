...
class _VaexColumn:
    ...
    def __init__(self, column : vaex.expression.Expression, metadata : dict = {}) -> None:
        ...
        if metadata["vaex.cetagories_bool"][self._col.expression]:
    ...
    def get_mask(self) -> Tuple[_VaexBuffer, Any]:
        ...
        if null == 0:
            ...
            if self._col.is_masked:
                buffer = _VaexBuffer(self._col.values.mask)
                _k = _DtypeKind
                _np_kinds = {'u': _k.UINT, 'b': _k.BOOL}
                kind = _np_kinds.get(self._col.values.mask.dtype.kind, None)
                if kind is None:
                    raise RuntimeError("Null representation is not a bit or byte mask.")
                bitwidth = self._col.values.mask.dtype.itemsize
                format_str = self._col.values.mask.dtype.str
                endianness = self._col.values.mask.dtype.byteorder
