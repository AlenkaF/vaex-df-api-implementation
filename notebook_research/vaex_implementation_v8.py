# Added mask handling:
# updated describe_null
# get_mask
# apply change to buffer_to_ndarray
# apply the mask to Vaex dataframe

# Added sentinel handling also
...
def convert_column_to_ndarray(col : ColumnObject) -> pa.array:
    """
    Convert an int, uint, float or bool column to a numpy/arrow array
    """
    if col.offset != 0:
    ...
    print(x)
    # If there are missing data with mask
...
def buffer_to_ndarray(_buffer, _dtype) -> np.ndarray:
    ...
    _bool = {1: bool, 8: bool} # Add 1 bitwidth case for bitmasks
    _np_dtypes = {0: _ints, 1: _uints, 2: _floats, 20: _bool}
    ...
    if bitwidth == 1: # Add this case for bitmasks
        x = np.ctypeslib.as_array(data_pointer,
                          shape=(_buffer.bufsize // (bitwidth),))
    else: 
        x = np.ctypeslib.as_array(data_pointer,
...
class _VaexColumn:
    ...
    def __init__(self, column : vaex.expression.Expression, metadata : dict = {}) -> None:
        ...
        The values of expression can be NumPy or Arrow.
        For now only deal with NumPy.
        """
        ...
        #if isinstance(column.values, (pa.Array, pa.ChunkedArray)):
        #    raise NotImplementedError("Value of columns of type {} not handled "
        #                              "yet".format(type(column.values)))
    ...
    def describe_null(self) -> Tuple[int, Any]:
        ...
        kind = self.dtype[0]
        value = None
        if kind in (_k.INT, _k.UINT):
            if self._col.is_masked or isinstance(self._col.values, (pa.Array, pa.ChunkedArray)):
                null = 4
            else:
                null = 0
        elif kind in (_k.FLOAT, _k.BOOL, _k.CATEGORICAL):
            if self._col.is_masked or isinstance(self._col.values, (pa.Array, pa.ChunkedArray)):
                null = 4
            else:
                null = 1  # None
    ...
    def get_data_buffer(self) -> Tuple[_VaexBuffer, Any]:  # Any is for self.dtype tuple
        ...
        if self.dtype[0] in (_k.INT, _k.UINT, _k.FLOAT, _k.BOOL):
            ...
            if isinstance(self._col.values, (pa.DictionaryArray)):
                # If I use indices from arrow dict something funny comes out from the buffer
            ...
            else: # in case of Vaex categorize
    ...
    def get_mask(self) -> Tuple[_VaexBuffer, Any]:
        ...
        if null == 0:
            ...
            mask_array = self._col.values 
            if self._col.is_masked:
                dtype = self._col.values.mask.dtype
                buffer = _VaexBuffer(mask_array.mask)
            elif isinstance(self._col.values, (pa.Array, pa.ChunkedArray)):
                dtype = self._col.values.is_null().to_numpy(zero_copy_only = False).dtype # experimental
                buffer = _VaexBuffer(mask_array.is_null().to_numpy(zero_copy_only = False))  # experimental

            _k = _DtypeKind
            _np_kinds = {'u': _k.UINT, 'b': _k.BOOL}
            kind = _np_kinds.get(dtype.kind, None)
            if kind is None:
                raise RuntimeError("Null representation is not a bit or byte mask.")
            bitwidth = dtype.itemsize
            format_str = dtype.str
            endianness = dtype.byteorder
        else:
            raise NotImplementedError('See self.describe_null')
            
        return buffer, (kind, bitwidth, format_str, endianness)
